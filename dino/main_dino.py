...
import jittor.dataset.dataset as datasets
...
def get_args_parser():
    ...
    parser.add_argument("--lr", default=0.008, type=float, help="""Learning rate at the end of
    ...
    # Misc
    parser.add_argument('--data_path', default="/home/zixuanqian/tmp/newdino/dataset/ImageNetS50/train/", type=str,
        help='Please specify path to the ImageNet training data.')
    parser.add_argument('--output_dir', default="./output3", type=str, help='Path to save logs and checkpoints.')
...
def train_dino(args):
    ...
    # temp = sam_encoder.state_dict()
    # for name, para in list(temp.items()):
    #     if name.startswith('blocks.'):
    #         if '.' in name:
    #             if name.split('.')[-2] in ['lin1', 'lin2']:
    #                 newname = name.replace('blocks.', '').replace('lin', 'fc')
    #                 temp[newname] = para
    #                 del temp[name]
    #             elif name.split('.')[-1] in ['rel_pos_h', 'rel_pos_w']:
    #                 del temp[name]
    #             else:
    #                 newname = name.replace('blocks.', '')
    #                 temp[newname] = para
    #                 del temp[name]
    #     if not name.startswith('blocks.'):
    #         del temp[name]
    # student.blocks.load_state_dict(temp, strict=False)
    # teacher.blocks.load_state_dict(temp, strict=False)
    ...
    if args.optimizer == "adamw":
        optimizer = jt.optim.AdamW(params_groups, lr=args.lr)  # to use with ViTs
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        temp = teacher.state_dict()
        temp2 = student.state_dict()
    ...
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with jt.no_grad():
            ...
            for param_q, param_k in zip(student.parameters(), teacher_without_ddp.parameters()):
                param_k.data.__mul__(m).__add__((1 - m) * param_q.detach().data)
...
class DINOLoss(nn.Module):
    def __init__(self, out_dim, ncrops, warmup_teacher_temp, teacher_temp,
        ...
        self.ncrops = ncrops
        self.register_buffer("center", jt.zeros(1, out_dim))
    ...
    def execute(self, student_output, teacher_output, epoch):
        ...
        teacher_out = jt.nn.softmax((teacher_output - self.center) / temp, dim=-1)
    ...
    def update_center(self, teacher_output):
        ...
        """
        batch_center = jt.sum(teacher_output, dim=0, keepdims=True)
        #jt.mpi.all_reduce(batch_center)
        batch_center = batch_center / (len(teacher_output) * jt.world_size)
