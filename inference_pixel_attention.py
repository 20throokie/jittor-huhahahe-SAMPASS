...
def parse_args():
    ...
    parser.add_argument("--dump_path", type=str, default='./vit_pass4/pixel_attention', help="The path to save results.")
    parser.add_argument("--data_path", type=str, default="/home/zixuanqian/tmp/ImageNetS50/ImageNetS50/", help="The path to ImagenetS dataset.")
    parser.add_argument("--pretrained", type=str, default="/home/zixuanqian/tmp/origin/vit_pass4/pixel_attention/checkpoint.pth.tar", help="The model checkpoint file.")
    ...
    parser.add_argument("-t", "--threshold", default=0.39, type=float, help="The threshold to filter the 'others' categroies.")
    parser.add_argument("--test", default=False, help="whether to save the logit. Enabled when finding the best threshold.")
    parser.add_argument("--centroid", type=str, default="/home/zixuanqian/tmp/origin/vit_pass4/pixel_attention/cluster/centroids.npy", help="The centroids of clustering.")
...
def main_worker(args):
    ...
    for images, path, height, width in tqdm(dataloader):
        ...
        with jt.no_grad():
            ...
            mask = nn.upsample(mask, (h, w), mode="bilinear", align_corners=False).squeeze(0).squeeze(0) # (h,w)
            ...
            logit = mask
            prediction = jt.argmax(cosine, dim=1, keepdims=True)[0] + 1 # argmax返回的第一个值是类别号
