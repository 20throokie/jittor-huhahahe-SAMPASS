...
def densecrf(image, mask):
    ...
    output_logits = jt.array(np.concatenate((bg, fg), axis=0))

    H, W = image.shape[:2]
    image = np.ascontiguousarray(image)

    output_logits = jt.nn.interpolate(output_logits.unsqueeze(0), size=(H, W), mode="bilinear").squeeze()
    output_probs = jt.nn.softmax(output_logits, dim=0).cpu().numpy()
